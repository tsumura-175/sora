...
各ページ固有のHTMLは `src/pages/<page>/main.html` にあります。お問い合わせの後にページ固有の要素が続く場合は `after-contact.html` にあります。タイトル、description、canonical、preload設定は `page.json` にあります。CSS、JavaScript、画像は `src/assets/` を編集します。
